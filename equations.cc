  ...
  dealii::Point<dim, dealii::VectorizedArray<double>>              q_point_loc) const
{
  ...
  //The components of n in three directions x,y,z
  ...
  scalarvalueType epsilon_1;
  epsilon_1 = constV(1.0) - constV(3.0) * constV(epsilon);

  a_n = epsilon_1 + constV(4.0) * constV(epsilon) * (nx * nx * nx * nx + ny * ny * ny * ny + nz * nz * nz * nz);
  ...
  scalarvalueType val_term2 =
    ...
    (constV((1.0 - k) / 2.0) / (tau_U * tau_U) *
     (phix[0] * (Dtilde * ((constV(1.0) - phi) / constV(2.0)) * Ux[0] + j_at[0]) +
      ...
      phix[2] * (Dtilde * ((constV(1.0) - phi) / constV(2.0)) * Ux[2] + j_at[2])));
  ...
  int random_number = rand() / (RAND_MAX + 1) - 0.5;  // Generates a random number between -0.5 and 0.5
  scalarvalueType rand_nose;
  rand_nose = eta * random_number * sqrt(constV(userInputs.dtValue));

  scalarvalueType eq_phi = (phi + constV(userInputs.dtValue) * xi / tau_phi) + rand_nose;
  ...
  dealii::Point<dim, dealii::VectorizedArray<double>>              q_point_loc) const
{
  ...
  aniso[0] = a_n * a_n * phix[0] ;
  aniso[1] = a_n * a_n * phix[1] ;
  aniso[2] = a_n * a_n * phix[2] ;

  scalargradType aniso_1;
  aniso_1[0] = constV(16.0) * normgradn * a_n * constV(epsilon) * nx * (ny * ny * (ny * ny - nx * nx) + nz * nz * (nz * nz - nx * nx));
  aniso_1[1] = constV(16.0) * normgradn * a_n * constV(epsilon) * ny * (nx * nx * (nx * nx - ny * ny) + nz * nz * (nz * nz - ny * ny));
  aniso_1[2] = constV(16.0) * normgradn * a_n * constV(epsilon) * nz * (nx * nx * (nx * nx - nz * nz) + ny * ny * (ny * ny - nz * nz));
  ...
  scalargradType eqx_xi = (-aniso-aniso_1);
